...
function makeHybridAgent(
  ...
  return new BrowserAgent({ executor, observer: new DOMPageObserver(), planner, validator: new DefaultActionValidator(), debugArtifacts: options.debugArtifacts });
}
...
test('ambiguous download is resolved by llm fallback', async () => {
  ...
  await agent.run('download main file', `${baseUrl}/ambiguous-download-choice.html`, 1);
...
test('semantic relevant link is chosen by llm fallback', async () => {
  ...
  await agent.run('open install docs', `${baseUrl}/semantic-link-choice.html`, 1);
...
test('main content is selected via llm fallback', async () => {
  ...
  const steps = await agent.run('extract main content', `${baseUrl}/multi-content-page.html`, 1);
...
test('latest list item is selected via hybrid planning', async () => {
  ...
  await agent.run('open latest item', `${baseUrl}/list-latest-item.html`, 1);
